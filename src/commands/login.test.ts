...
jest.mock('../utils.js', () => {
  return {
    prompt: jest.fn(),
    actionRunner: (fn: (...args: unknown[]) => unknown) => fn,
    serializeError: (e: unknown) => JSON.stringify(e),
  };
});
jest.mock('../config.js');
jest.mock('../forma.js');
...
const mockExchangeIdAndTkForAccessToken =
  exchangeIdAndTkForAccessToken as jest.MockedFunction<
...
describe('login command', () => {
  ...
  it('should open browser and prompt for magic link', async () => {
    const magicLink =
      'https://joinforma.page.link/?link=https%3A%2F%2Fclient.joinforma.com%2Flogin%2Fmagic%3Fid%3Dtest-id%26tk%3Dtest-tk';
