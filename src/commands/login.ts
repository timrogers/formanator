...
const promptForEmailedMagicLink = (
  ...
  try {
    ...
    return promptForEmailedMagicLink(e as any);
  }
};
...
command
  ...
  .action(
    actionRunner(async () => {
      console.log('We need to authenticate you with Forma.');
      console.log(
        'A browser window will open to https://client.joinforma.com/login?type=magic',
      );
      console.log('Please enter your email address there to receive a magic link.');
      console.log('Once you receive the email, copy the magic link and paste it here.');
