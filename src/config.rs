...
///
/// On macOS: token goes to Keychain, other config (email, timestamps) goes to TOML file
/// On other platforms: all config including token goes to TOML file
pub fn store_config(config: &Config) -> Result<()> {
    ...
    let serialised = toml::to_string(&config_to_write)?;
